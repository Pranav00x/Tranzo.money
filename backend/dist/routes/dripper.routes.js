...
router.get("/", requireAuth, async (req, res) => {
    try {
        const role = req.query.role === "employer" ? "employer" : "employee";
...
router.post("/:id/withdraw", requireAuth, async (req, res) => {
    try {
        const result = await StreamService.withdrawFromStream(req.params.id, req.user.sub);
        res.json(result);
    }
    catch (err) {
        res.status(400).json({ error: err.message });
...
router.post("/:id/cancel", requireAuth, async (req, res) => {
    try {
        const result = await StreamService.cancelStream(req.params.id, req.user.sub);
        res.json(result);
    }
    catch (err) {
        res.status(400).json({ error: err.message });
...
//# sourceMappingURL=dripper.routes.js.map
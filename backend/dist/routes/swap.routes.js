...
router.post("/quote", requireAuth, async (req, res) => {
    try {
        ...
        const quote = await SwapService.getQuote(params.fromToken, params.toToken, params.amount, params.chainId, req.user.smartAccount);
 ...
 */
router.post("/execute", requireAuth, sensitiveLimiter, async (req, res) => {
    try {
        const params = executeSwapSchema.parse(req.body);
        const result = await SwapService.executeSwap(req.user.sub, params.fromToken, params.toToken, params.amount, params.chainId, params.slippageBps);

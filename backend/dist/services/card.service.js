...
async function partnerFreezeCard(partnerId) {
    // TODO: await kulipaClient.freezeCard(partnerId)
}
async function partnerUnfreezeCard(partnerId) {
    // TODO: await kulipaClient.unfreezeCard(partnerId)
}
async function partnerUpdateLimits(partnerId, dailyLimit, monthlyLimit) {
    // TODO: await kulipaClient.setSpendingLimits(partnerId, { daily: dailyLimit, monthly: monthlyLimit })
}
function verifyWebhookSignature(_headers, _rawBody) {
...
export class CardService {
     ...
     */
    static async orderCard(userId, type, deliveryAddress) {
     ...
     * to be signatureless.
     */
    static async activateCardOnChain(userId, cardId, spendLimitEth) {
        ...
        const { sessionKeyPK, setupHash } = await SessionKeyService.createAndInstallSessionKey(user.signerPrivateKey, spendLimitEth);
     ...
     */
    static async processCardPayment(cardNumber, cvv, merchantAddress, amountWei) {
        ...
        const userOpHash = await SessionKeyService.executeWithSessionKey(card.user.smartAccount, card.sessionKeyPK, merchantAddress, amountWei, card.spendLimit);
     ...
     */
    static async getCardTransactions(userId, limit = 50, offset = 0) {
     ...
     */
    static async updateLimits(userId, cardId, dailyLimit, monthlyLimit) {
        const card = await this.assertCardOwner(userId, cardId);
        ...
        if (card.partnerId) {
            await partnerUpdateLimits(card.partnerId, dailyLimit ?? card.dailyLimit, monthlyLimit ?? card.monthlyLimit);
        }
        const updated = await prisma.card.update({
            where: { id: cardId },
            data: {
     ...
     */
    static async handleAuthorizationWebhook(payload, rawHeaders, rawBody) {
        ...
        try {
            ...
            const balances = await BalanceService.getBalances(user.smartAccount, ENV.DEFAULT_CHAIN_ID);
        ...
        if (user.signerPrivateKey && usdcAddress) {
            setImmediate(async () => {
                try {
                    ...
                    const txHash = await SmartAccountService.sendGaslessTransaction(user.signerPrivateKey, usdcAddress, 0n, usdcTransferData, ENV.DEFAULT_CHAIN_ID);
                    await Promise.all([
                    ...
                    // Push notification
                    await NotificationService.notifyCardTransaction(user.id, merchantName, amount);
                }
                catch (err) {
    ...
    static async handleSettlementWebhook(payload) {
        ...
        if (existingTx) {
            ...
            if (status === "completed") {
                await NotificationService.notifyTransactionConfirmed(existingTx.userId, txHash ?? "", "card_settlement");
            }
        }
        console.log(`[Card Settlement] Partner tx ${transactionId} → ${mappedStatus}`);
    }
    // ─── Helpers ────────────────────────────────────────────────────────────────
    static async assertCardOwner(userId, cardId) {
        const card = await prisma.card.findUnique({ where: { id: cardId } });
        if (!card)
            throw new AppError(404, "Card not found");
        if (card.userId !== userId)
            throw new AppError(403, "Not your card");
        return card;
    }

...
async function fetchQuoteFrom0x(params) {
    ...
    if (!res.ok) {
        ...
        throw new AppError(res.status, `0x API error: ${err.reason ?? res.statusText}`);
    }
...
export class SwapService {
     ...
     */
    static async getQuote(fromToken, toToken, amount, chainId = ENV.DEFAULT_CHAIN_ID, takerAddress) {
     ...
     */
    static async executeSwap(userId, fromToken, toToken, amount, chainId = ENV.DEFAULT_CHAIN_ID, slippageBps = 50 // 0.5% default slippage
    ) {
        ...
        const quote = await this.getQuote(fromToken, toToken, amount, chainId, user.smartAccount);

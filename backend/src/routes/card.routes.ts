...
router.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const card = await CardService.orderCard(
        req.user!.sub,
        type,
        deliveryAddress
      );
...
router.put(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const card = await CardService.updateLimits(
        req.user!.sub,
        cardId,
        dailyLimit,
        monthlyLimit
...
router.get(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const { transactions, total } = await CardService.getCardTransactions(
        req.user!.sub,
        limit,
        offset
      );
...
router.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      ...
      console.log(
        `[Card Auth] ${payload.merchantName} $${payload.amount} — ${result.approved ? "APPROVED" : "DECLINED"} (${elapsed}ms)`
      );

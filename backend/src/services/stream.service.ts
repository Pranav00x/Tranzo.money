...
export class StreamService {
  ...
  static async createStream(params: {
    ...
    const totalAmount = (
      BigInt(params.amountPerSecond) * BigInt(totalDurationSec)
    ).toString();
   ...
   */
  static async withdrawFromStream(streamId: string, userId: string) {
    ...
    if (
      stream.employeeAddress.toLowerCase() !==
   ...
   */
  static async cancelStream(streamId: string, userId: string) {
    const stream = await prisma.stream.findUniqueOrThrow({
      where: { id: streamId },
    });
   ...
   */
  static async listStreams(userId: string, role: "employer" | "employee") {
    const user = await prisma.user.findUniqueOrThrow({
      where: { id: userId },
    });
